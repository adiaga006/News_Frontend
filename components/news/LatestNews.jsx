...
const LatestNews = () => {
    ...
    const latest_news_get = async () => {
        const cachedNews = sessionStorage.getItem('latest_news');
        
        if (cachedNews) {
            setNews(JSON.parse(cachedNews)); // Use cached news if available
        } else {
            try {
                const res = await fetch(`${base_api_url}/api/latest/news`);
                const data = await res.json();
                setNews(data.news);
                sessionStorage.setItem('latest_news', JSON.stringify(data.news)); // Store news in session storage
            } catch (error) {
